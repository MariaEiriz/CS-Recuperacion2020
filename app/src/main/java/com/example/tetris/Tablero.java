...
public class Tablero {
    ...
    private static final int alturaTablero = 20;
    private static final int anchuraTablero = 10;
    private static int tab[][] = new int[anchuraTablero][alturaTablero];
    private final Random random = new Random();
    private static final ArrayList<Pieza> listaPiezas = new ArrayList<>();
    ...
    public boolean puedeMoverse(Pieza pieza, int x, int y, boolean vengoDeRotar) {
        ...
        for (Point a : puntos) {
            if ((a.x < anchuraTablero && a.x >= 0 && a.y >= 0 && a.y < alturaTablero && (tab[a.x][a.y] == 0 || tab[a.x][a.y] == 8))
                    ||((a.equals(new Point(pieza.x1, pieza.y1)) || a.equals(new Point(pieza.x2, pieza.y2))
